        ...
        if self.cell.noise:
            ...
            for t in range(self.seq_len):
                ...
                if len(self.cell.full_K) == 0:
                    ...
                    if lag - self.attn_window > 0:
                        K = tf.concat([past_K[:,:,:(lag-self.attn_window)], state.K], axis=-2)
                        V = tf.concat([past_V[:,:,:(lag-self.attn_window)], state.V], axis=-2)
                    else:
                        K = state.K
                        V = state.V
                    #print("max LAG, attn_window", max(lag+1, self.attn_window +1))
                    #print("len of sequence of keys", K.shape[-2])
                    #print("-"*50)
